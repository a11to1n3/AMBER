...
def _ambr_git_revision() -> Optional[str]:
    """Revision of the *AMBER package*, never the caller's CWD checkout.

    Order:

    1. ``AMBER_GIT_REVISION`` environment variable (CI / release inject).
    2. Build-time constant ``ambr._build_info.GIT_REVISION`` when present
       (optional module generated by packaging; may be absent in editable
       installs).

    Deliberately does **not** run ``git rev-parse`` in the process CWD — that
    reports whichever repo the user happens to be sitting in, not AMBER.
    """
    env = os.environ.get("AMBER_GIT_REVISION")
    if env:
        return env.strip() or None
    try:
        from . import _build_info  # type: ignore

        sha = getattr(_build_info, "GIT_REVISION", None)
...
def _application_revision() -> Optional[str]:
    """Caller-supplied application revision (separate from AMBER's SHA)."""
    env = os.environ.get("AMBER_APP_REVISION")
...
def build_run_info(
    ...
    *,
    ...
    info: Dict[str, Any] = {
        ...
        # AMBER package revision (env / build-time only — never CWD git).
        "git_revision": _ambr_git_revision(),
        # Caller application revision (explicitly separate).
