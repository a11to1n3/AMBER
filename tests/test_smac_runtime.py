...
def test_smac_optimizer_deterministic_when_model_seed_pinned():
    ...
    opt = SMACOptimizer(
        ...
        seed=0,
        strategy="random",
        fixed_params={"n_agents": 4, "steps": 1, "seed": 0, "show_progress": False},
    )
    assert opt.deterministic is True
    assert bool(opt.scenario.deterministic) is True
    result = opt.optimize()
    hist = result["history"]
    drop = [c for c in ("cost", "objective", "time", "trial") if c in hist.columns]
    assert hist.drop(drop).unique().height == hist.height
