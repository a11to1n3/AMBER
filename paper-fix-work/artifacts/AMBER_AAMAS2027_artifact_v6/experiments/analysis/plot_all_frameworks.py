...
def write_md(path_src, data, by):
    ...
    w("semantic/monitor package on host_a (RTX 3090). The 10M AMBER/FLAME endpoints")
    ...
    w("```")
    w()
    w("## Relation to host_a campaign")
    w()
    w("| Campaign | Host | Focus | Report |")
    w("|---|---|---|---|")
    w("| **All frameworks (this file)** | RTX **5090** | 10 frameworks × 4 models × scale | this MD |")
    w("| Semantic / monitor package | RTX **3090** (`host_a`) | C1–C4 attestation, activation, overhead | `DETAILED_RESULTS_host_a.md` |")
    w()
