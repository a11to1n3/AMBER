#!/usr/bin/env bash
# FLAME GPU 2 Schelling benchmark on CUDA host (RTX 5090).
set -euo pipefail
...
# NVRTC needs curand headers under CUDA_PATH/include (vast.ai images omit them).
...
cd "$ROOT"
# run_all_frameworks.py lives in benchmarks/; on vast.ai we mirror that layout.
mkdir -p results
