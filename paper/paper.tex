...
This audit surfaced four distinct correctness bugs in the checked-in benchmark implementations of SimPy (wealth race condition, missing boundary clamp, hardcoded 100\%-infected SIR initial condition) and Agents.jl (hardcoded step count mismatch). A later review also aligned SimPy, Melodie, and Agents.jl on the same independent x/y movement kernel used by AMBER, AgentPy, and Mesa, and made the Agents.jl row use the same trimmed-mean run protocol as the Python rows. The timings reported below reflect every framework passing every invariant.
...
This audit surfaced four correctness bugs in the checked-in benchmark implementations---three in SimPy (wealth race condition, missing boundary clamp, hardcoded 100\%-infected SIR initial condition) and one in Agents.jl (hardcoded step count mismatch). A later review also aligned the random-walk/SIR movement kernel across SimPy, Melodie, Agents.jl, AMBER, AgentPy, and Mesa, and made the Agents.jl row use the same trimmed-mean run protocol as the Python rows. The evaluation in Section~(3) reflects every framework passing every invariant.
