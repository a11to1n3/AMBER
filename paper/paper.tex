...
\begin{abstract}
Agent-based models are often written in Python for accessibility, but object-per-agent execution limits the population sizes that can be explored interactively or in parameter sweeps. AMBER is a Python framework that stores agent state in a Polars-backed columnar table and exposes population operations through a compact view API. This design preserves familiar model and agent abstractions while lowering common updates to compiled column operations. Per-agent logic remains available through a buffered object-oriented path for behaviours that do not vectorise. We evaluate AMBER on wealth transfer, random walk, and spatial SIR benchmarks against Mesa, AgentPy, SimPy, Melodie, Agents.jl, and AMBER's own loop path, with correctness checks that verify comparable model outputs before timing. AMBER is the fastest Python-hosted implementation on every tested model, reaching speedups of up to 1118$\times$ over Mesa and outperforming Agents.jl on the largest SIR benchmark.
\end{abstract}
...
Python has become the dominant platform for ABM in the research community, primarily through Mesa \citep{masad2015mesa} and AgentPy \citep{foramitti2021agentpy}. Their accessibility has enabled widespread adoption, but both, along with most Python-hosted ABM tools, share an architectural assumption that limits scalability: each agent is a Python object, and population-wide updates are Python-level loops over those objects. For populations of thousands to tens of thousands of agents, the interpreter overhead of this pattern can dominate the cost of the model itself, constraining parameter sweeps, Monte Carlo replicates, and model complexity.

AMBER addresses this bottleneck through two architectural choices. First, agent state is centralised in a columnar DataFrame backed by Polars \citep{polars_zenodo}, a high-performance data processing library implemented in Rust. Second, a \emph{view API} for full-population, filtered, and scatter updates compiles user-facing expressions down to a small number of Polars operations, so users write Python while the expensive work executes in compiled code. A conventional object-oriented \texttt{Agent} class remains available for per-agent logic that does not vectorise.

This paper makes three contributions. First, it presents a columnar state architecture for Python-hosted ABM that keeps the standard model/agent interface while moving population operations into compiled columnar kernels. Second, it introduces a view abstraction that supports full-population, filtered, and id-indexed scatter updates without exposing users to low-level DataFrame plumbing. Third, it provides a seven-framework benchmark suite with correctness checks that caught and corrected multiple cross-framework implementation mismatches before performance was reported.

\subsection{Motivation}

Researchers who build large ABMs often face a trade-off between accessibility and speed. They can stay in Python and accept the interpreter-overhead ceiling of Mesa or AgentPy, or they can move to Java, Julia, or GPU-oriented C++ and absorb the cost of a second language, a different toolchain, and a narrower pool of collaborators. The first choice caps practical population sizes; the second increases the engineering burden around otherwise domain-focused models.

AMBER targets the middle ground: Python source code, familiar ABM abstractions, and compiled execution for the population-wide operations that dominate many simulations. Models expressed in Mesa or AgentPy translate cleanly because AMBER preserves the usual model, agent, and agent-list hierarchy. The main conceptual shift is that updates are expressed as views over the population rather than loops over individual agents.
...
AMBER is organised around a columnar state store, a user-facing view API, and conventional ABM components for environments, experiments, and optimisation. The design goal is to make the fast path explicit without abandoning the object-oriented abstractions that make Python ABM code readable.
...
We evaluate AMBER against six other agent-based modelling frameworks. The evaluation combines wall-clock timing with a correctness audit that verifies comparable model outputs before timing is interpreted.
...
AMBER (vectorized) is the fastest Python-hosted framework on all three models and wins the largest SIR benchmark outright. Agents.jl wins the two cheaper microbenchmarks, where Julia's compiled dispatch has less fixed overhead than repeated Python-to-Polars calls. The results separate into three regimes.
...
AMBER is designed as a general-purpose ABM framework for three overlapping groups of users.

\textbf{Domain scientists building mid-to-large-scale models.} Epidemiologists simulating disease transmission across contact networks, ecologists modelling species distributions or predator-prey dynamics, economists studying market formation or financial contagion, and social scientists investigating opinion dynamics can express their models in AMBER using the view API at population sizes comfortably in the tens of thousands. Existing Mesa or AgentPy models port with limited structural change: the \texttt{Model}/\texttt{Agent} class hierarchy is preserved, while population-wide updates move from per-agent loops to \texttt{agents.where(...)} expressions. The OOP path lets graph traversal, Schelling-style satisfaction checks, rule-based strategic behaviour, and other non-vectorised logic coexist with the vectorised core.
...
\textbf{Educators teaching ABM.} The legacy per-agent API remains supported and offers a familiar starting point for students new to agent-based thinking. Once a model is working, the same class can be migrated incrementally to the view API as an example of vectorisation and performance engineering. The documentation at \url{https://amber.readthedocs.io} provides a quickstart tutorial, walkthroughs of three canonical models, and API reference material generated from source docstrings.

Support is provided through the GitHub issue tracker at \url{https://github.com/a11to1n3/AMBER/issues}. Contributions in the form of bug reports, pull requests, and new examples are welcome; the contributing guide in the repository describes the development workflow and code-review criteria.

\section{Conclusion}

AMBER shows that a Python ABM framework can retain familiar modelling abstractions while moving the dominant population operations into a compiled columnar backend. The benchmark results identify both the strengths and limits of this design: columnar execution gives large gains over object-per-agent Python frameworks and can outperform a compiled Julia baseline on heavier spatial workloads, while very small per-agent kernels remain better suited to low-overhead compiled loops. The result is a practical design point for researchers who need scalable agent-based simulation without leaving the Python ecosystem.
