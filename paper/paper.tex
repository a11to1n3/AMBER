...
The \textbf{Random Walk} model has agents move in continuous 2D space according to random velocity vectors with boundary clamping. This tests position update kernels that are fully vectorizable.

\subsubsection{Correctness Verification}

Runtime numbers are meaningful only if every framework is computing the same thing. Before any timing runs, we execute each framework against a dedicated correctness script (\texttt{benchmarks/correctness\_check.py}) that checks per-model output invariants:
...
This audit surfaced four distinct correctness bugs in the checked-in benchmark implementations of SimPy (wealth race condition, missing boundary clamp, hardcoded 100\%-infected SIR initial condition) and Agents.jl (hardcoded step count mismatch). All four were fixed before timing was performed; the timings reported below reflect every framework passing every invariant.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{scaling_chart.png}
    \caption{Execution time versus agent population size for three benchmark models, all seven frameworks. Lower is better. AMBER (vectorized) and Agents.jl track each other closely on every model; every other framework trails significantly.}
    \label{fig:scaling}
\end{figure}
...
\midrule
AMBER (vectorized) & 4 & 5 & 17 & 3 & 2 & \textbf{5} & 102 & \textbf{141} & \textbf{608} \\
Agents.jl (Julia) & \textbf{1} & \textbf{1} & \textbf{7} & \textbf{1} & \textbf{1} & 7 & \textbf{5} & 40 & 892 \\
AMBER (loop) & 9 & 17 & 89 & 8 & 16 & 79 & 196 & 857 & 11{,}470 \\
SimPy & 18 & 37 & 205 & 20 & 40 & 209 & 100 & 528 & 6{,}750 \\
Melodie & 17 & 33 & 168 & 96 & 190 & 963 & 374 & 1{,}170 & 9{,}990 \\
AgentPy & 26 & 51 & 266 & 10 & 20 & 98 & 156 & 809 & 8{,}780 \\
Mesa & 35 & 116 & 2{,}870 & 9 & 18 & 87 & 188 & 728 & 9{,}180 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Agents.jl (Julia)  & 0.4$\times$ (slower) & 1.4$\times$ & 1.5$\times$ \\
AMBER (loop)       & 5.2$\times$  & 15.8$\times$ & 18.9$\times$ \\
SimPy              & 12.1$\times$ & 41.8$\times$ & 11.1$\times$ \\
Melodie            & 9.9$\times$  & 192.6$\times$ & 16.4$\times$ \\
AgentPy            & 15.6$\times$ & 19.6$\times$ & 14.4$\times$ \\
Mesa               & 168.8$\times$ & 17.4$\times$ & 15.1$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
AMBER (vectorized) wins two of three models outright and comes within 2.4$\times$ of Agents.jl on the third. The story each row tells is slightly different.

\textbf{Random Walk.} AMBER wins at 5 ms vs Agents.jl's 7 ms (a 1.4$\times$ advantage) because both the position update and the boundary clamp are expressible as two numpy/Polars kernels with essentially zero per-step overhead. Julia's JIT compiler and Polars' SIMD path are neck-and-neck on this workload; what tips it toward AMBER is that Polars amortizes its query-planning cost more effectively across a tight body of two arithmetic operations than Julia's interpreted dispatch does against compiled kernels. Every other framework is 10--200$\times$ slower because their per-agent Python loop dominates the actual floating-point work.

\textbf{SIR Epidemic.} AMBER wins at 608 ms vs Agents.jl's 892 ms (1.5$\times$). The infection step is quadratic in population size because each infected agent must distance-test every susceptible agent. AMBER expresses this as a Polars cross join filtered by the squared-distance predicate, which runs as one compiled pipeline in Rust; Agents.jl does the same computation as a nested loop that the Julia compiler specializes per-call but which still processes one pair at a time at the assembly level. At 5{,}000 agents the Polars pipeline pays off; at the smaller sizes Agents.jl wins because its per-call overhead is near zero.

\textbf{Wealth Transfer.} Agents.jl wins at 7 ms vs AMBER's 17 ms (2.4$\times$). The per-step work is so small --- one array comparison, one random draw per active agent, two scatter-adds --- that Polars' per-query overhead (query planning, Python--Rust boundary crossings, metadata bookkeeping) becomes the dominant cost. Julia's compiled dispatch wins the microbenchmark because there is essentially no computation for Polars to amortize over. Notably, AMBER is still more than $10\times$ faster than every other Python-hosted framework on this workload, and the AMBER (loop) path alone beats Mesa by a factor of 32.

\textbf{AMBER loop vs Mesa on wealth transfer}: Mesa's 5{,}000-agent result (2.87 s) is 168$\times$ slower than AMBER (vectorized) and 32$\times$ slower than AMBER (loop). This gap reflects Mesa 3.x's \texttt{AgentSet.shuffle\_do} materializing the full agent list per step for shuffle-ordered activation, compounded with the scheduler's per-agent dispatch overhead. The wealth-transfer workload happens to be a worst case for this pattern because every agent acts every step.

\textbf{When the Julia JIT wins.} The wealth-transfer microbenchmark is instructive: it establishes the regime where a compiled JIT language has a structural advantage over a Python-hosted columnar framework. When the per-step work is $O(\mathrm{constant})$ per agent with essentially no amortizable work, no amount of SIMD vectorization closes the Python-to-Rust call overhead gap. For any non-trivial agent logic (SIR, random walk with actual computation) the compiled kernel amortizes that overhead and AMBER draws level or ahead. In practice, the wealth-transfer pattern appears as a unit test more often than as a real research workload.

\section{(4) Quality Control}

\subsection{Testing}

AMBER maintains an automated test suite of 230 unit and integration tests executed on every push to the \texttt{main} and \texttt{dev} branches via GitHub Actions. Line coverage of the source package is 73\%. Tests are organised into:

\begin{itemize}
\item \textbf{Unit tests} (\texttt{tests/test\_\{agent,model,sequences,population,base\}.py}) covering the view API's attribute read/write protocol, the write-buffer flush semantics, \texttt{add\_agents} bulk creation, filtered and scatter view construction, and all legacy list-style compatibility surfaces.
\item \textbf{Environment tests} (\texttt{tests/test\_environments.py}) covering grid, continuous-space, and network environments, including a regression guard for a torus-wrap neighbour-deduplication bug fixed as part of this release.
\item \textbf{Integration tests} (\texttt{tests/test\_integration.py}) covering end-to-end simulation workflows. The file deliberately keeps two class suites: \texttt{TestFullSimulationWorkflows} exercises the legacy per-agent loop path as a backwards-compatibility regression guard, while \texttt{TestVectorizedWorkflows} exercises the view API idiom.
\item \textbf{Optimisation tests} (\texttt{tests/test\_optimization.py}) covering the grid-, random-, Bayesian-, and SMAC-based calibration paths.
\end{itemize}
...
Runtime numbers are meaningful only if every framework is computing the same thing. Before any timing run, AMBER executes each framework implementation against a dedicated correctness script (\texttt{benchmarks/correctness\_check.py}) that checks per-model output invariants:
...
This audit surfaced four correctness bugs in the checked-in benchmark implementations---three in SimPy (wealth race condition, missing boundary clamp, hardcoded 100\%-infected SIR initial condition) and one in Agents.jl (hardcoded step count mismatch). All four were fixed as part of this release before any timing was performed. The evaluation in Section~(3) reflects every framework passing every invariant.
...
\begin{description}
\item[Code repository.] Name: AMBER. Identifier: \url{https://github.com/a11to1n3/AMBER}. Licence: BSD 3-Clause. Date published: 2024. Version published at time of writing: \texttt{v0.2.0}.

\item[Archive.] Name: Zenodo. Identifier: \texttt{10.5281/zenodo.XXXXXXX} (to be minted on acceptance via the GitHub--Zenodo integration). Licence: BSD 3-Clause. Publisher: Anh-Duy Pham. Version published: \texttt{v0.2.0}. Date published: April 2026.
\end{description}
